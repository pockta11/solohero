...
| E1-06 | v1 → v2 이관 경로를 구현한다 | v1 데이터를 가진 계정이 진행도(g 인덱스)·장비·**강화 지출 골드(환급)**를 잃지 않고 진입. 강화 레벨은 의미가 바뀌어(가산→승산) 복사하지 않는다 | M |
...
**목표** 등급별 확률표 + 천장 가챠를 구현하고, 게임 내 확률 공시가 실측과 일치하게 만든다.

**데모 조건** 100회 연속 뽑기에서 Legendary가 최소 1회 보장되고, 10만 회 실측 분포가 공시표와 오차 1%p 이내로 일치한다.

| ID | 스토리 | 완료 기준 | 우선순위 |
|---|---|---|---|
| E5-01 | 확률표 누적 합 추첨을 구현한다 (GACHA_RATE 4행) | 단위 테스트로 경계값(0, 0.55, 0.88, 0.98, 0.999)이 기대 등급으로 매핑 | M |
| E5-02 | 천장 리셋 규칙을 구현한다 (Legendary 획득 시 리셋, 데이터 플래그) | 자연 Legendary·천장 Legendary 모두 pity 0으로, 플래그 false 시 천장만 리셋 | M |
| E5-03 | 확률표 데이터 검증을 구현한다 (합 100 %, 음수 없음) | 에디터 검증기와 부트 시 검사에서 잘못된 표를 거부 | M |
| E5-04 | 천장을 구현한다 (100회 도달 시 등급 판정 생략, Legendary 확정) | pity 100번째 뽑기가 항상 Legendary. 리셋 동작은 E5-02 규칙을 따름 | M |
| E5-05 | 슬롯 균등 선택을 구현한다 | 4슬롯이 각 25%로 선택됨 (10만 회 실측 검증) | M |
...
| E5-12 | 천장 진행도를 UI에 상시 노출한다 | `천장까지 N회` 게이지가 실제 pity 값과 일치 | M |
| E5-13 | 확률 공시표를 게임 내에서 열람 가능하게 한다 | 등급 4행 확률 + 천장·리셋 규칙이 표시되고, 추첨에 쓰는 동일 데이터에서 그려짐 | M |
| E5-14 | 검증기로 10만 회 실측하고 결과를 기록한다 | 등급별 실측 비율이 공시표와 1%p 이내, 슬롯 25% 확인 | M |
| E5-15 | 가챠 로직 단위 테스트를 작성한다 | 확률표 추첨·천장 보장·리셋 규칙 3항목 커버 | M |
