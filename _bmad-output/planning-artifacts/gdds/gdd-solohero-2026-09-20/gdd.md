...
게임의 심장은 두 가지다. 첫째, 앱을 끈 시간이 골드로 환산되는 **오프라인 축적**(최대 6시간). 둘째, 등급별 확률을 게임 안에서 공시하고 100회에 Legendary를 보장하는 **투명한 천장 가챠**. 플레이어는 확률표와 `천장까지 N회` 게이지로 다음 Legendary까지의 거리를 항상 본다. 가챠 방식 자체는 장르 표준(등급별 확률표 + N회 천장)을 따른다.
...
| **P-B. 확률 분석러** (보조 타깃) | 가챠 확률 구조에 흥미, 기댓값·천장 계산을 즐기고 커뮤니티에 공유 | 확률이 투명하게 공시되고, 천장까지의 거리와 기댓값을 직접 계산할 수 있는 구조 | pity 진행도 게이지 상시 노출, 확률 공시표를 게임 내에서 열람 가능 |
...
1. **천장까지 몇 번인지 항상 보인다.** 등급별 확률(Common 55 / Rare 33 / Epic 10 / Legendary 2 %)을 게임 안에서 공시하고, `천장까지 N회` 게이지를 가챠 패널에 상시 노출하며, 100회 천장에서 Legendary를 보장한다. 중복은 즉시 골드로 환급되어 뽑기가 헛되지 않는다. 방식은 장르 표준이고, 차별점은 **숨기는 것이 없다**는 점이다.
...
| G-4 | 가챠 확률을 검증 가능한 형태로 공시한다 | 게임 내 공시표 = 실제 확률표. 검증기 실측(10만 회) 오차 1%p 이내 |
...
**계승과 폐기가 명확히 갈린다.** 천장 가챠의 구조(100회 보장·4슬롯×4등급·자동 장착), 오프라인 보상, 광고 2배, 저장 아키텍처(원격 + 로컬 백업 + 디바운스), 빌드 파이프라인, Firebase/AdMob 인프라는 계승한다. 3D 전투(NavMesh·구체 판정), 가로 레이아웃 UI 구현, 3D 에셋, 선형 강화·스테이지 공식, 그리고 **정규분포 기반 가챠 등급 결정**(아키텍처 단계에서 장르 표준 확률표로 대체)은 폐기한다.
...
| **P3** | **확률이 보이는 가챠** (Transparent Gacha) | 등급별 확률과 천장 규칙을 게임 안에서 공시하고, 천장까지 남은 횟수를 항상 보여준다. 플레이어가 기댓값을 계산할 수 있어야 한다 | 숨겨진 확률, 확률 미공시, 천장 진행도 비노출, 공시표와 다른 실제 확률 |
...
**등급 수는 v1.0에서 4단계로 유지한다.** 레퍼런스 게임은 7단계지만, 등급을 늘리면 확률표 4행과 천장 구조를 다시 설계해야 한다. 5번째 등급 **Mythic**은 v1.1에서 확률표를 5행으로 재배분하며 추가한다.
...
2. 뽑기는 슬롯을 먼저 균등 확률(각 25%)로 고르고, 등급을 **확률표**로 고른다. 슬롯과 등급이 정해지면 장비는 1종으로 확정된다.
3. **등급 확률표 (v1.0 확정):** Common **55 %** / Rare **33 %** / Epic **10 %** / Legendary **2 %**. 합은 항상 100 %이며, 이 표가 그대로 게임 내 공시표다. pity 누적은 확률을 바꾸지 않는다 — 방식은 장르 표준(등급별 확률표 + N회 천장)이다.
4. 등급 추첨은 [0, 1) 균일 난수 하나를 확률표의 누적 합과 비교해 결정한다. 다른 분포·가중치·보정은 쓰지 않는다.
5. **천장:** pity 카운터가 100에 도달하는 뽑기는 등급 판정을 건너뛰고 Legendary를 확정 지급한다.
6. **천장 리셋:** Legendary를 획득하면 천장·자연 획득을 가리지 않고 pity 카운터를 0으로 리셋한다 (장르 표준). 천장 전 자연 Legendary로 카운터가 유지되던 초안 규칙(D-017)은 폐기한다.
7. pity 카운터는 1회·10연 구분 없이 뽑기 1건당 1 증가한다. 10연은 10건으로 계산되며 그 안에서 천장이 걸릴 수 있다.
8. **확률 공시:** 가챠 패널에 `확률 공시` 버튼을 두고, 등급별 확률 4행과 천장·리셋 규칙을 게임 내에서 열람할 수 있게 한다. 공시표는 실제 추첨에 쓰는 확률표와 같은 데이터에서 그려지며, 검증기 실측(10만 회)과 오차 1%p 이내여야 한다.
9. **진행도 노출:** 가챠 패널 상단에 `천장까지 N회` 게이지를 상시 표시한다. 필러 P3의 핵심 표현이다.
...
**DoD**
- [ ] 확률표 합 100 %·천장 보장·Legendary 획득 시 리셋이 각각 단위 테스트로 검증된다
- [ ] 10만 회 실측 등급 분포가 공시표와 오차 1%p 이내로 일치한다
...
| V-5 | 확률표 4행이 장비 획득 페이스(첫 Epic·첫 Legendary 도달 시점)를 목표 곡선과 맞추는가. 실측 분포가 공시표와 1%p 이내인가 | GACHA_RATE | Epic·Legendary 비율을 조정한다. 천장 100은 고정 |
...
| 가챠 | `pityCount`, `totalPullCount` | 가챠 패널 천장 게이지 | 카드 뒤집기, 등급 파티클, `천장까지 N회` |
...
| **E5** | 가챠 | 확률표 추첨·천장·리셋·1회/10연·중복 환급·확률 공시 | E4 | 100회 뽑기에서 Legendary가 보장되고 실측 분포가 공시표와 일치한다 |
...
| 가챠 분포 일치 | 10만 회 실측 대비 공시표 오차 1%p 이내 | 출시 게이트 |
...
| 가챠 | 등급별 확률표(55/33/10/2) + 100회 천장 + Legendary 획득 시 리셋, 1회/10연, 연출 5단계, 천장 진행도 노출, 확률 공시표 |
...
| Mythic 등급 (5번째 등급) | 확률표 5행 재배분·천장 구조 재검토가 필요 | v1.1 |
...
| 정규분포 가챠 등급 결정 (Box-Muller · μ 이동 · 최근접 매칭 · 비리셋 pity) | 아키텍처 단계에서 장르 표준 **등급별 확률표 + 100회 천장 + Legendary 획득 시 리셋**으로 대체 (decision-log D-049~D-052) |
